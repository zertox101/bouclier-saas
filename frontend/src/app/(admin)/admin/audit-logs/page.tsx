...
export default function AuditLogsPage() {
    ...
    return (
        <div className="space-y-6 p-6">
            <div className="flex items-center gap-4"><FileText className="w-6 h-6 text-purple-400" /><h1 className="text-2xl font-bold text-white">Audit Logs</h1></div>
            <div className="bg-slate-900/50 border border-slate-800 rounded-lg overflow-hidden">
                <table className="w-full text-xs">
                    ...
                    <tbody>{(logs.length > 0 ? logs : []).map((log, i) => (
                        <tr key={i} className="border-t border-slate-800 hover:bg-slate-800/30">
                            <td className="p-3 text-slate-400 font-mono">{log.timestamp || log.created_at || log.time || ""}</td>
                            <td className="p-3 text-white">{log.user || log.username || ""}</td>
