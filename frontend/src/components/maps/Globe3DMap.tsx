...
export type GlobeViewMode = 'Global Map' | 'Regional View' | 'Botnet Path' | 'Tactical Hub' | '2D Flat Map';
...
export default function Globe3DMap({ attacks: initialAttacks, timeRangeLabel = "15m", viewMode = 'Global Map' }: Globe3DMapProps) {
    ...
    useEffect(() => {
        ...
        switch (viewMode) {
            case 'Regional View':
                // Zoom in on target (Europe/Paris)
                globeRef.current.pointOfView({ lat: TARGET.lat, lng: TARGET.lng, altitude: 0.8 }, 2000);
                break;
            case 'Botnet Path':
                // Zoom out to see wide arcs
                globeRef.current.pointOfView({ lat: 20, lng: 0, altitude: 2.5 }, 2000);
                break;
            case 'Tactical Hub':
                // Tilt view for tactical feel
                globeRef.current.pointOfView({ lat: 10, lng: 20, altitude: 1.8 }, 2000);
                break;
            default: // Global Map
                globeRef.current.pointOfView({ lat: 20, lng: 0, altitude: 2.0 }, 2000);
        }
    }, [viewMode]);
        ...
        for (const f of flows) {
            ...
            if (!prev) {
                agg.set(key, {
                    ...
                    topRuleId: f.rule_id,
                    bytesOut: 0,
                    type: (f as any).type // Pass through the type (e.g. SENTINEL_GUARD_ALERT)
                });
            } else {
                prev.count += 1;
                ...
                // If it's a Sentinel Guard alert, ensure it's marked
                if (((f as any).type === "SENTINEL_GUARD_ALERT") || (f.rule_id?.startsWith("AI_"))) {
                    (prev as any).type = "SENTINEL_GUARD_ALERT";
                }
    ...
    const pointsData = useMemo(() => {
        const points = attackPoints.map((a) => {
            const isAI = (a as any).type === "SENTINEL_GUARD_ALERT" || a.topRuleId?.startsWith("AI_");
            const sevColor = isAI ? "#a855f7" : // Purple for AI Attacks
            ...
            return {
                lat: a.lat,
                lng: a.lng,
                size: isAI ? 2.5 : Math.min(0.6 + a.count / 100, 2),
                color: sevColor,
                label: `
                    <div style="font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; padding:10px; border-radius:12px; background:rgba(10,10,20,.95); border:1px solid ${isAI ? 'rgba(168,85,247,.5)' : 'rgba(34,211,238,.25)'}; min-width: 220px; box-shadow: 0 0 20px ${isAI ? 'rgba(168,85,247,.2)' : 'rgba(0,0,0,0.5)'}">
                        ${isAI ? `<div style="font-[900]; color:#a855f7; font-size: 10px; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 2px;">⚡ SENTINEL GUARD EVENT</div>` : ""}
    ...
    return (
        <div className="relative w-full h-full overflow-hidden bg-transparent flex flex-col">
            <div ref={containerRef} className="relative flex-1 w-full h-full overflow-hidden">
                ...
                {mounted && (
                    <Globe3D
                        ...
                        // HexBin (Heatmap/Density) - Only visible in Global/Tactical
                        hexBinPointsData={viewMode === 'Botnet Path' ? [] : hexPoints}
