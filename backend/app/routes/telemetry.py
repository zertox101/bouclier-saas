...
def _resolve_org_id(request: Request, token: Optional[str] = None) -> str:
    """Resolve org_id from X-Organization-ID header or JWT token. Falls back to 'default'."""
...
def ingest_event(evt: IngestEvent, background_tasks: BackgroundTasks, db: Session = Depends(get_db), request: Request = None):
    org_id = _resolve_org_id(request) if request else "default"
...
def heartbeat(hb: Heartbeat, background_tasks: BackgroundTasks, db: Session = Depends(get_db), request: Request = None):
    org_id = _resolve_org_id(request) if request else "default"
...
@router.get("/stream")
async def sse_stream(request: Request, channels: str = "events,health,kpi", token: Optional[str] = Depends(oauth2_scheme_optional)):
    """
    Unified SSE Endpoint.
    Channels: comma-separated list of streams to subscribe to.
    """
    org_id = _resolve_org_id(request, token)
...
@router.get("/stats")
def get_telemetry_stats(request: Request, db: Session = Depends(get_db), token: Optional[str] = Depends(oauth2_scheme_optional)):
    """
    Returns aggregated stats for the SOC Dashboard.
    Cached in Redis for 10 seconds to prevent DB overload.
    Allows public access for demo reliability.
    """
    org_id = _resolve_org_id(request, token)

    # ── Redis cache check (fast path) ──────────────────────────────────────────
    CACHE_KEY = f"telemetry_stats:{org_id}"
    ...
    heatmap = [[int(t[1] or 0)//3, 6-int(t[0] or 0), t[2] or 0] for t in heatmap_query]
    ...
    if not any(severity_counts.values()):
        ...
        if not timeline_list:
            if is_sqlite:
                hour_func_ae = func.strftime("%H:00", AlertEvent.timestamp)
            else:
                hour_func_ae = func.to_char(AlertEvent.timestamp, "HH24:00")
            alert_timeline_records = db.query(
                hour_func_ae.label("hour"),
                AlertEvent.id
    ...
    try:
        ...
        logger.debug(f"CACHE_WRITE | {CACHE_KEY} | TTL={CACHE_TTL}s")
    except Exception as cache_err:
...
def generate_telemetry_report(
    ...
    """
    Generates a professional SOC report based on telemetry data.
    
    Args:
        report_type: Type of report (executive, technical, incident)
        format: Output format (html, json, pdf)
        db: Database session
    
    Returns:
        Professional SOC report in requested format
    """
...
@router.get("/sensors")
def list_sensors(request: Request, db: Session = Depends(get_db), token: Optional[str] = Depends(oauth2_scheme_optional)):
    from app.core.security import decode_access_token
    org_id = _resolve_org_id(request, token)
    ...
    return db.query(TelemetrySensor).filter(TelemetrySensor.org_id == org_id).all()
