                                    ...
                                    <div className="col-span-4 flex flex-col gap-6">
                                        <AuditSection title="Deep-Packet Inspection Stream" icon={Activity} className="h-[600px] overflow-hidden">
                                            <div className="h-full bg-black/60 rounded-2xl border border-white/5 p-4 font-mono text-[9px] space-y-1 overflow-y-auto custom-scrollbar relative">
                                                ...
                                                {[...Array(60)].map((_, i) => (
                                                    <div key={i} className="flex gap-4 group hover:bg-white/5 transition-colors p-0.5 rounded">
                                                        <span className="text-slate-600 w-16">[{new Date().toLocaleTimeString().split(' ')[0]}]</span>
                                                        <span className="text-blue-500 w-12 italic">ACK</span>
                                                        <span className="text-slate-400 uppercase truncate flex-1">
                                                            {i % 8 === 0 
                                                                ? <span className="text-red-400 font-black">!! CRITICAL_OVERFLOW_DETECTED :: ADDR_0x{btoa(auditTarget.ip).substring(0,6)}</span> 
                                                                : `STREAM_ID_${i.toString().padStart(3, '0')} :: LEN_${1024 + (i * 12) % 500} :: WIN_4096`}
                                                        </span>
                                                    </div>
                                                ))}
                                            ...
                                            <div className="flex flex-col gap-4">
                                                ...
                                                <button 
                                                    onClick={async (e) => {
                                                        ...
                                                        try {
                                                            await fetch(`http://localhost:8005/api/forensics/isolate/${auditTarget.label}`, { method: 'POST' });
                                    ...
                                    <div className="col-span-4 flex flex-col gap-6">
                                        ...
                                        <AuditSection title="Satellite Coordinate Mapping" icon={Navigation} className="h-[300px]">
                                            <div className="h-full bg-blue-500/5 border border-white/10 rounded-2xl relative overflow-hidden flex items-center justify-center group">
                                                <div className="absolute inset-0 opacity-20 bg-[url('//unpkg.com/three-globe/example/img/earth-topology.png')] bg-cover group-hover:scale-110 transition-transform duration-[10s]" />
