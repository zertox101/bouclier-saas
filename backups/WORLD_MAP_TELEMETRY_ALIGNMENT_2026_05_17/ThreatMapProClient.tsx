...
export default function ThreatMapProClient() {
    ...
    useEffect(() => {
        ...
        const fetchInitial = async () => {
            try {
                // Fetch points from redis stream for high-fidelity real-time map
                const resPoints = await fetch(`${API}/map/points?limit=100`);
                let mapData = [];
                if (resPoints.ok) {
                    const data = await resPoints.ok ? await resPoints.json() : { points: [] };
                    mapData = (data.points || []).map((p: any) => ({
                        id: p.id,
                        source: {
                            ip: p.ip,
                            country: p.country,
                        ...
                        details: {
                            ...
                            method: 'Suspicious Traffic',
                            severity: p.severity === 'critical' ? 'Critical' : 'High',
                            threat_level: p.severity === 'critical' ? 'Critical' : 'High'
                        }
                    }));
                }

                // Fetch summary for sidebar alerts
                const resSummary = await fetch(`${API}/api/soc-expert/summary`);
                if (resSummary.ok) {
                    const data = await resSummary.json();
                    const latest = (data.latest_alerts || []).map((a: any) => ({
                        id: a.id,
                        source: {
                            ip: a.src_ip,
                    ...
                    setEvents([...latest, ...mapData].slice(0, 50));
                }
            ...
            <header className="absolute top-0 left-0 right-0 h-24 border-b border-white/5 bg-black/40 backdrop-blur-2xl flex items-center justify-between px-10 z-[110]">
                ...
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-10 border-x border-white/5 px-10">
                        <HeaderMetric label="Latency" value="12ms" color="text-emerald-400" />
                        <HeaderMetric label="Sat_Link" value="Active" color="text-blue-400" />
                        <HeaderMetric label="Neural_Sync" value="99%" color="text-p-400" />
                    </div>
                ...
                <aside className="absolute left-8 top-32 w-80 space-y-4 z-10">
                    <div className="bg-[#080C14]/80 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-6 shadow-4xl">
                        ...
                        <div className="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
                            {events.map((ev) => (
                                <div key={ev.id} onClick={() => setSelectedEvent(ev)} className={cn("p-4 rounded-2xl border transition-all cursor-pointer group", selectedEvent?.id === ev.id ? "bg-blue-600/10 border-blue-500/40" : "bg-white/[0.02] border-white/[0.05] hover:bg-white/5")}>
            ...
            <footer className="absolute bottom-0 left-0 right-0 h-24 border-t border-white/5 bg-black/40 backdrop-blur-2xl flex items-center justify-between px-10 z-[110]">
                <div className="flex items-center gap-12">
                    <div className="flex flex-col">
                        <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest mb-1">Total_Intercepts_24H</span>
                        <span className="text-2xl font-black text-white font-mono">142,084</span>
                    </div>
                    <div className="flex flex-col border-l border-white/5 pl-12">
                        <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest mb-1">Global_Threat_Index</span>
                        <span className="text-2xl font-black text-red-500 font-mono">CRITICAL</span>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                    ...
                    <button className="px-10 py-4 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] shadow-xl shadow-blue-600/30 hover:scale-[1.02] transition-all">
