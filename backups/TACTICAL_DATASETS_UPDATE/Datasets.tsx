...
import { 
  ...
  Biohazard, Factory, ShieldAlert, CheckCircle,
...
const DATASETS = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Intrusion Detection Evaluation Dataset (CIC-IDS 2017)", year: 2017, type: "Classic" }
    ]
  },
  {
...
export default function Datasets() {
  ...
  useEffect(() => {
    fetch("http://localhost:8005/api/datasets")
      .then(res => res.ok ? res.json() : [])
      .then(data => Array.isArray(data) ? setBackendDatasets(data) : [])
  ...
  const handleIntegrate = async (name: string) => {
    setIntegrating(name);
    try {
      const res = await fetch(`http://localhost:8005/api/datasets/integrate/${encodeURIComponent(name)}`, { method: "POST" });
                          ...
                          <GlassCard key={idx} className="p-6 group relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-3 pt-4 border-t border-white/5">
                              <button 
                                ...
                                disabled={integrating === mergedItem.name}
                                className={cn(
